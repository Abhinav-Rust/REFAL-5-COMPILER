...
| Self-hosting fixpoint | 13.0% | 8.0 | C1 = C2 = C3 at 12,599 bytes over the full grammar, every generation checked, **and the driven fixpoint now gates the Refal driver**: `the_refal_driver_reaches_a_fixpoint_on_the_compiler_itself` drives the compiler's own 132 KB source with `compiler.ref`'s own `RESIDUALIZE-DRIVEN`, requires the residue to be checked Refal, drives it again, and requires byte-identity and equality with the Rust oracle's residue. The row withheld credit because the driven fixpoint gated the Rust driver rather than the Refal one; it now gates the Refal one. Credit is still withheld for the compiler's *default* path, which normalises |
...
fixpoint that depends on it — hold 38.5 of the 100 points and carry 11.5 of the 24
