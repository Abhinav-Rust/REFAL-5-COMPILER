...
**Gate:** for every corpus program, symbolic `drive → clean → generalise → residualise → run` agrees with the Phase 1 interpreter on all test inputs. **Partly green:** the `residual` corpus mode drives 29 programs, re-checks every residue as Refal, and requires its output to equal the interpreter's. What is not yet covered is the generalisation half — a program whose argument is wholly unknown still stops driving instead of case-splitting, so no corpus row exercises the whistle path end to end.
