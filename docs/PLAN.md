...
| Graph of states / Refal emission | 8.5% | 6.0 | **T-4, T-5, T-6, T-9** all closed and gated, and **the compiler's default path now drives**, so the stage that compiles pattern matching is the stage the compiler is. What the product lacks is that residualization is bounded rather than whole-program for general programs: past the step budget the driven path refuses rather than falling back to a call it retains |
...
fixpoint that depends on it — hold 38.5 of the 100 points, and this revision pays
7.5 of the 17 that were deducted from them. **The runtime has left the deducted
group entirely.** It was the repository's largest engineering item, and the two
shapes it was still deducting for are both closed and measured: the unbalanced
rope, and — found by following the measurement rather than the plan — a bracket
that deep-copied its contents on every pattern that opened it. What remains
concentrated is that residualization is *bounded* rather than total: past its
step budget the driven path refuses rather than falling back to a call it
retains, so a program that needs more driving than the budget allows is not
compiled. That, §4.4's strategy search, §6.4's `unknown` values and the
release/conformance evidence are the remaining 17 points. The figure agrees with
the milestone table in the README, which is the point: a reader counting ticks
and a reader reading the percentage should reach the same conclusion about where
the work is.
