...
| T-8 | Programs are **data**: metacode representation | Ch. 1.3 *Representations and Metacodes* | 1980 §1.3; Ch. 6 §6.2; 1975 *REFAL macrocode* | `Dn`/`Up` implement the manual's metacode table | ✅ Done for ground expressions — `Dn` rewrites only the asterisk (`'a*b'` metacodes to `a*Vb`, the manual's own example) and keeps brackets; `Up` inverts it, **activates** the calls it recovers (`<Up '*'((F)'abc')> == <F 'abc'>`, the manual's worked example), reproduces deferred metacode `'*!'(E0)` verbatim, and rejects a free-variable metacode as Exercise 6.2 requires. `examples/metacode-chapter6.ref` exercises all five behaviours in the CLI corpus. §6.4 `unknown` values remain open: every runtime `Value` is ground |
