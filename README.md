...
| Refal machine / runtime | 19.5% | 12.5 | Broad covered builtin suite, no fixed call-depth limit, the projecting matcher (§2.2), and bindings now carried as a shared immutable spine rather than a map deep-copied per nested term. **The heap-allocated single view field is still missing** — the difference between a Refal machine and a work-list interpreter over host recursion — and the cost of that is now *measured* rather than assumed: `refal run` over an n-term input is super-quadratic, 63 B in 0.5 s, 9.4 KB in 17 s, 47.5 KB in 587 s, because every step copies the remaining expression into a binding and copies it again into the next call's argument list. Block sentences carrying conditions still take the recursive path |
