...
[#7](../../issues/7)); a non-trivial program transformer written in Refal (T-1) —
`examples/transformer-rename.ref` is the first that is not a compiler, and the
differential that would close the objective is still to come; §4.4's compilation
strategy, which is now *selectable* but not yet *searched*; §6.4's `unknown`
values for metacoding non-ground expressions; `driver.ref`; native code
