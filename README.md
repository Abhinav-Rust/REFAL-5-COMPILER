...
| Refal machine / runtime | 19.5% | 14.0 | Broad covered builtin suite, no fixed call-depth limit, the projecting matcher (§2.2), and **the view field's first half**: a binding is now a *range* in a shared arena rather than an owned run of terms, so `s.C e.Rest` costs O(1) in the length of the expression instead of O(n), and a frame whose result is exactly one run propagates it rather than materialising it. Measured, because a speedup that is not measured is a claim: the compiler's own 47.5 KB source went from 587 s to 299 s, and 63 B from 0.5 s to 0.38 s. **The second half is still missing** — a result that is a prefix followed by a call (`s.C <StripCR (e.CR) e.R>`, which is how Refal writes a list walk and is the compiler's hottest loop) is still flattened once per level, which is why the gain is a factor of two and not twenty. Block sentences carrying conditions still take the recursive path |
...
| Compiler implemented in Refal | 25.5% | 13.0 | A real Refal-authored lexer, parser, checker and emitter, byte-identical to the Rust bootstrap on every lowerable example, and three stages of the *transforming* half now live in `compiler.ref` as verified differentials against `refal-core`: the §4.2 seed graph (55/55), residualization (55/55) and the ground driver, which contracts the entry configuration and reproduces `refal drive`'s step count, visited-state trace and output. What is still Rust is the half that makes it a compiler rather than a reporter: **symbolic driving** — case splitting, folding, generalization — and the compilation of pattern matching. **The largest single deduction** |
...
| 4 | Refal machine | 🔶 Partial | Broad covered builtin suite: arithmetic, file I/O (`Card`/`Open`/`Get`/`Put`/`Putout`), buried data (`Br`/`Dg`/`Cp`/`Rp`/`Dgall`), structural ops (`First`/`Last`/`Lenw`/`Lower`/`Upper`), `Arg`/`Step`/`Time`/`Mu`/`Dn`/`Up`/`Trunc`/`Real`, plus `Prout`/`Print`/`Explode`/`Implode`/`Ord`/`Chr`/`Numb`/`Symb`/`Type`; backtracking, conditions, blocks in both positions. **No fixed call-depth limit** (work-list driven, 50,000 frames under a second) and the **projecting matcher** (§2.2) are done | Heap-allocated single view field — the work list still copies term slices, so this is a work-list interpreter over host recursion rather than Turchin's flat view-field rewriting machine, and the cost is measured: `refal run` is super-quadratic in the input's length (63 B in 0.5 s, 9.4 KB in 17 s, 47.5 KB in 587 s), which is what makes the self-hosting gate the slowest test in the repository; §6.4 `unknown` metacode values. Block sentences carrying conditions still take the recursive path |
...
with no fixed call-depth limit and the projecting matcher (§2.2); blocks end-to-end in both
positions; an explicit worklist evaluator; `refal-core` graph infrastructure with bounded
...
**Not yet done (~13% of 1.0 target):** the *symbolic* half of the transforming
...
which is the same gap; **the heap-allocated single view field**, so the machine
still copies term slices instead of rewriting one flat view — now measured at
63 B in 0.5 s, 9.4 KB in 17 s and 47.5 KB in 587 s, which makes it both the
largest engineering item and the reason the self-hosting gate is slow; §4.4's
compilation strategy, which is now *selectable* but not yet *searched*; §6.4's
`unknown` values for metacoding non-ground expressions; native code generation
(§4.7, deliberately after self-hosting).
...
| `refal-runtime` | 🔶 Broad covered builtin suite; worklist drives named calls and blocks with no fixed depth cap; projecting matcher (§2.2); Chapter 6 metacode table for `Dn`/`Up` | **Heap-allocated single view field, so the term sequence is copied by the work list rather than rewritten in place; §6.4 `unknown` metacode values** |
