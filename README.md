...
transition (T-9)**; **clean graphs (T-6)** — `refal clean` removes every sentence whose
quasiinput set is empty, `refal perfect` reports the §4.5 verdict, and the corpus gate
re-checks and re-runs the cleaned residue; and a Refal-authored lexer, parser, checker and
emitter — the lexer tokenises Classic Refal-5 and its own source, the parser builds an AST
...
**Not yet done (~13% of 1.0 target):** heap-allocated single view field (issue
[#7](../../issues/7)); full generalization (§4.6, the 1988 algorithm); metacodes (Ch. 1.3,
Chapter 6 contract); bracket contents in the format lattice; a non-trivial program
