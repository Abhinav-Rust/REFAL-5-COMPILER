...
| Graph of states / Refal emission | 8.5% | 6.0 | T-4, T-5, T-6 and T-9 are closed and gated, and **the compiler's default path now drives**: `refal compile` contracts the entry configuration and emits the program the driven graph denotes, so the pattern matching the source decided at run time is decided at compile time. A recursion is unrolled, a block is resolved and a pair of conditions is discharged, in Refal, by a compiler written in Refal. What the *product* still lacks is that residualization is bounded rather than whole-program for general programs: past the step budget the driven path refuses rather than falling back to a call the residue retains |
...
the 100 points, and this revision pays 7.5 of the 17 that were deducted from them. **The
...
What it is **not** yet is a compiler whose residualization is total: past its step budget
the driven path refuses rather than falling back to a call it retains, so a program that
needs more driving than the budget allows is not compiled. That, §4.4's strategy search,
§6.4's `unknown` values and the release/conformance evidence are what the remaining 17
points are. That is the honest summary, and 83% is what it scores.
...
**Not yet done (~17% of 1.0 target):** whole-program residualization for general
programs — past its step budget the driven path refuses rather than falling back
to a call the residue retains, so a program needing more driving than the budget
allows is not compiled; §4.4's compilation strategy, which is *selectable* but
not yet *searched*; §6.4's `unknown` values for metacoding non-ground
expressions; a clause-by-clause Classic Refal-5 conformance corpus and release
packaging; and native code generation (§4.7, deliberately after self-hosting).
