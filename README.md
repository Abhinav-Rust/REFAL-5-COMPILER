...
| Refal machine / runtime | 19.5% | 19.0 | Broad covered builtin suite, no fixed call-depth limit, the projecting matcher (§2.2), and **the view field, all three of its shapes**: a binding is a *range* of a shared arena; a frame's result is a **rope of runs of shared arenas** which the matcher consumes without flattening it; and **a bracket's contents are a run of that arena too**, so opening a bracket is a reference count rather than a deep copy of everything inside it. That last one was the repository's largest hidden cost — a Refal program passes its lists in brackets, so *every* list-passing pattern paid it once per call, and the graph pass copied millions of records merely to look at them. The rope is also balanced now: `Concat` carries a height and `concat` rotates on a left-heavy join, so appending no longer builds a spine whose depth every later pop has to pay. Both were measured: the compiler's own 132 KB source went from *not finishing* to **24.0 s for `GRAPH`** and **36.5 s for `RESIDUALIZE-DRIVEN`**, byte-identical to the Rust oracle. What is left is that block sentences carrying conditions still take the recursive path, and that §6.4's `unknown` metacode values are open |
| Graph of states / Refal emission | 8.5% | 6.5 | T-4, T-5, T-6 and T-9 are closed and gated, **the compiler's default path now drives**, and **residualization is total**: a call reached with the driving budget spent is left residual rather than aborting the compiler, so the budget bounds the number of driven states and not whether a program comes out. What the *product* still lacks is §4.4's compilation strategy — selectable but not yet searched |
...
the 100 points, and these two revisions pay 9 of the 15 that were deducted from them. **The
runtime has left the deducted group entirely**: it was the repository's largest engineering
item, and the shapes it was still deducting for — an unbalanced rope, and a bracket that
copied its contents — are both closed and measured. Any future change to the figure has to
move one of those two rows, because they are where the product actually is.
...
fixpoint — and **the Refal-authored compiler's default path drives**, which is the claim the
previous revision of this paragraph had to withhold. `refal compile` contracts the entry
configuration and emits the program the driven graph denotes: a recursion is unrolled, a
block whose subject is a literal is resolved away, and a pair of conditions is discharged,
in Refal, by a compiler written in Refal. Its output is checked Refal and it answers what
the source answered, which `refal differential --compiled` proves by running it. That gate
found a real defect on its first run — `Up` activates the calls a metacoded expression
denotes, so a residue that dropped a definition reachable only that way failed at run time
where the original succeeded — and the fix is one predicate shared by both implementations.

What it is **not** yet is a compiler whose compilation strategy is chosen rather than fixed.
It no longer refuses a program because its budget was tight — residualization is total, so
the budget bounds how much is driven and not whether a program comes out — and what remains
is that §4.4's strategy is *selectable* but not *searched*, §6.4's `unknown` values are open,
the front end has no clause-by-clause conformance corpus, and there is no release packaging.
That is the honest summary, and 85% is what it scores.
The full weighting rationale is in [`docs/PLAN.md`](docs/PLAN.md).
...
in particular had drifted (the corpus is 57 lowerable examples, not the 47 the previous
...
| 5 | Graph of states | 🔶 Partial | Seed graph, SCC, structural cleanup, bounded ground driver, shape-aware symbolic driver, homeomorphic-embedding whistle, **case splitting on a wholly unknown argument**, bounded Tier 1 analysis (`refal analyze`, `refal overlap`), cleaned-graph Core Refal emitter, driven/generalized residualization that is total -- a call reached with the budget spent is left residual rather than aborting, **T-4 closed** — `drive → clean → residualise` verified against the interpreter over 30 corpus programs, **T-9 closed** — a metasystem transition demonstrated, **T-6 closed** — `refal clean` removes every sentence whose quasiinput set is empty (§4.3) and `refal perfect` reports the §4.5 verdict, with the cleaned residue re-checked and re-run by the corpus gate, **T-5 closed** — neighborhoods (1988 §3), generalization by common history (§2), and the §4 loop-back rule as a selectable strategy | §4.4's compilation strategy is selectable but not yet searched |
...
selectable via `--strategy`; and a Refal-authored lexer, parser, checker and emitter — the
lexer tokenises Classic Refal-5 and its own source, the parser builds an AST and parses the
lexer, and C1 = C2 = C3 self-hosting holds at 12,599 bytes over the full Classic grammar.
The **transforming** half is in Refal too now: `GRAPH`, `RESIDUALIZE`, `DRIVE`,
`DRIVE-SYMBOLIC` and `RESIDUALIZE-DRIVEN` are all byte-identical to their `refal-core`
counterparts over the corpus, and **the last of them is the compiler's default path**, so
`refal compile` drives. CI green.

**Not yet done (~15% of 1.0 target):** §4.4's compilation strategy, which is
*selectable* but not yet *searched*; §6.4's `unknown` values for metacoding
non-ground expressions; a clause-by-clause Classic Refal-5 conformance corpus and
release packaging; the compiler's speed on very large inputs; and native code
generation (§4.7, deliberately after self-hosting).
...
| Date | Change |
|---|---|
| 2026-09-26 | Fifty-fourth milestone: **residualization is total, so the compiler always emits a program.** The driven path had a budget of 10000 steps and *refused* when it ran out: `Err(DriveError::StepLimit)` in `refal-core`, `driven residualization error: step limit` in `compiler.ref`. That made the compiler's ability to compile a property of the budget rather than of the program. A call reached with the budget spent is now **left residual** — the verdict the driver already gives a call it cannot decide — so the residue keeps the call and the transitive retention carries its definition. The budget bounds the number of *driven* states; it no longer bounds whether a program comes out. `residualize-driven --steps 1` emits the source program, `--steps 3` emits `Go { = <Prout <Reverse 'c'> 'b' 'a'>; }`, `--steps 8` emits `Go { = <Prout 'c' 'b' 'a'>; }` — all three output-equivalent to what they came from, established by *running* the residue rather than by inspecting it (`residualization_is_total_when_the_budget_runs_out`, budgets 1, 2 and 5 over five fixtures). **The Refal side needed the budget to be reachable at all**: `RESIDUALIZE-DRIVEN` now takes it as a second argument and applies it by *seeding the step counter* at `10000 - N`, so the check `DsInvoke0` already made is the only place the budget exists and the 52 sites that rebuild the driver's context are untouched; the report subtracts the seed back, so a run that took three steps says `steps: 3`. Without that, the budget-exhausted arm would be written, unexercised and wrong the first time it mattered, because the default budget is never reached on the corpus. `the_refal_authored_driven_residualizer_is_total_when_the_budget_runs_out` byte-compares the two implementations at budgets 1, 2, 5 and 12. The `(ERR)` arms no longer mean "step limit" — they cannot, since it is no longer an error — and now say `undecided call`. The figure moves **~83% → ~85%**: the graph-of-states row takes 6.5 of 8.5, the Refal compiler row 23.5 of 25.5, and the self-hosting row 11.5 of 13.0 |
| 2026-09-26 | Fifty-third milestone: **the compiler's default path drives.** `Compile` was `Emit(Check(Parse(Lex(source))))` — it never touched the driver, which was the whole of the "Compiler implemented in Refal" deduction and the last thing three workstream rows were waiting on. It now drives: `refal compile` contracts the entry configuration and emits the program the driven graph denotes. The change is observable, not declared. `<Prout <Reverse 'abc'>>` comes back as `<Prout 'c' 'b' 'a'>`, a block whose subject is a literal is resolved away, and a pair of conditions is discharged — at compile time, in Refal, by a compiler written in Refal. **The normalising path is kept as its own mode**, `refal normalize`, with its own CLI differential against `lower`, because that is the path the Rust bootstrap's `lower` is a second implementation of; four older tests that compared the *default* output against `lower` were re-pointed at it rather than deleted. **A new gate found a real bug on its first run.** `refal differential --compiled` compares a program's runtime output against the driven residue rather than the lowered one, which is what makes it a deployability gate: agreeing with `lower` says the compiler is a correct printer, agreeing with the source says the compiled program runs. Over 21 examples covering literals, calls, recursion, conditions, backtracking, brackets, blocks, builtins, metacode and the Refal-body subset, all equal. It immediately caught `examples/metacode-chapter6.ref` compiling to a residue that called `Echo` without defining it — `function Echo was not found` at run time. The retention walk already knew `Mu` makes it unsound, because `Mu` applies a function whose name arrives as data; **`Up` is the same hazard one level down**, since it activates the calls a metacoded expression denotes and `((Echo) 'Z')` is a symbol inside a bracket rather than a call term. Both are now one predicate — `activates_a_carried_call` in `refal-core`, `DsRdActivates`/`DsRdActivatesL` in `compiler.ref` — so a residue that can still apply a carried name keeps every definition the original had. Two more gates land with it: `the_compiled_path_is_not_the_lowered_path` (driving has to be observable, or "compiled" is "lowered" wearing a new name) and `the_driven_compiler_resolves_a_block_at_compile_time`. The figure moves **~76% → ~83%**: the Refal compiler row takes 22.5 of 25.5, the self-hosting row 11.0 of 13.0, and the graph-of-states row 6.0 of 8.5. What is left is that residualization is *bounded* rather than total — past its step budget the driven path refuses instead of falling back to a call it retains |
| 2026-09-26 | Fifty-second milestone: **the graph pass is linear, and the view field reaches inside brackets.** Three changes, and the order they were made in is the finding. `CleanG` on the compiler's own 1,160-state graph was **462 s of the 480 s graph pass** against 0.56 s for the Rust `residualize-driven`, so it was rewritten first: from a state the walk reaches *every* state of the same function, so the reachable set is a union of whole functions; and `BuildG` emits a transition's target as `first_states[Upper callee]`, so a transition's target id **is** the callee's canonical id. Reachability is therefore reachability on the call graph — 480 small integers compared by symbol equality rather than 1,160 nested records compared by string — and every list the pass joins is already ordered by the key it is joined on, so each join became a **merge join**. `RenumS` and `RenumT` followed: the source remap is a merge join against the id-ordered map, and the target remap needs only the group heads, one entry per function rather than one per state. **The rewrite did not fix it.** What fixed it was looking at what the runtime does when a pattern opens a bracket: `Value::Bracket` held an owned `Vec<Value>`, so *opening a bracket deep-copied everything inside it*, nested records included — and since a Refal list walk is written `(e.Rest)`, that was not an edge case but the language's dominant shape. A bracket's contents are now a **`Slice`**, a run of a shared arena with an offset, so opening a bracket is a reference count and rebuilding one from a binding is one too. `Value::Bracket` still reads as `&[Value]` through a `Deref`, so `.iter()`, indexing and `.len()` are unchanged. Third, the rope: `Concat` carried no height, so appending built `((a ++ b) ++ c) ++ d` and reaching its head cost the spine's depth. It now carries a height and `concat` rotates on a left-heavy join — **only** left-heavy, because a right-heavy one is `s.C <Recurse ...>`, the shape the view-field invariants are stated in and the one whose head is reached in a single step, so it is preserved rather than rewritten. The rotation is taken only when the split is exact, which cost one bug: a clamped field reports its node's height, so the first version subtracted a left-child length the field never reaches and twelve tests panicked with `attempt to subtract with overflow`. They found it because they execute the paths. Fourth, a sentence with no conditions now takes its first match directly instead of materialising every split of every expression variable. **Measured on the compiler's own 132 KB source, release build: `GRAPH` 24.0 s and `RESIDUALIZE-DRIVEN` 36.5 s — both byte-identical to the Rust oracle, `GRAPH` line for line and `RESIDUALIZE-DRIVEN` at the same 51 steps — where `GRAPH` did not finish in 480 s and `RESIDUALIZE-DRIVEN` was killed at 10 m 28 s.** `RESIDUALIZE` is 34.6 s and `DRIVE` 23.4 s. This clears the number the previous `NEXT ACTION` set for wiring `Compile`: the driven path had to come in under a minute, and it is 36.5 s. **And the driven fixpoint now gates the Refal driver rather than the Rust one**: `the_refal_driver_reaches_a_fixpoint_on_the_compiler_itself` drives the compiler's own 132 KB source with `compiler.ref`'s own `RESIDUALIZE-DRIVEN` — 95,733 bytes, byte-identical to `refal residualize-driven` — requires it to be checked Refal, drives it again, and requires byte-identity. That was the one stated reason the self-hosting row was withholding credit. It is the repository's slowest test at 215 s and it is the point of the exercise. The figure moves **~72% -> ~76%** — the runtime row takes 19.0 of its 19.5 points, because both shapes it was deducting for are closed and measured, and the self-hosting row 8.0 of its 13.0, because the fixpoint it was waiting for now gates its own driver. Tests 319 -> 320 |
| 2026-09-25 | Fifty-first milestone: **the compiler is drivable, and the driver's cost is now a profile.** `refal residualize-driven examples/compiler.ref` was one step returning the program, because `Go`'s entry state was `('CHECK') (e.Source)` and the driver refuses to partition an entry whose pattern is anything more specific than a single bare expression variable. Moving the mode table into a `Dispatch` function and leaving `Go { e.Args = <Dispatch e.Args>; }` fixes that: **51 steps, 0.56 s, a 92,102-byte residue that `refal check` accepts**, with `Split1` … `Split8` being the CLI's mode dispatch compiled into a decision tree. Driving the driven compiler is **byte-identical** (99 steps, 92,011 bytes), so the fixpoint is a fixpoint of the *driver* rather than of a normaliser — and `the_driven_compiler_is_a_fixpoint_of_the_driver` now gates it in 1.9 s, with a non-vacuity assertion that compares the residue against `lower` and therefore fails if the entry is ever narrowed again. **The cost, which was the blocker, is now measured rather than guessed, and two of the three candidates the previous order ranked first are falsified.** The context's growing lists are not the cost — a 1,000-element context field costs the same as an empty one over 20,000 context rebuilds, because the view field already makes a bracket a shared range — and `DsScan` walking every state is not the cost either, at 351 calls on a 25-function chain. **98% of a `RESIDUALIZE-DRIVEN` run is the graph pass and 2% is the driving loop**, and the cost is a quadratic *comparison* count where every comparison is a function call: `MemberL` is 27–38% of all calls (the visited-set scan inside `CleanG`'s reachability walk), then `SameChars` at 13%, `SameFunc3` 11%, `TransFrom` 9%, `FuncName` 5%. Three shape defects were fixed on the way, all three invisible in the output because the answer stayed right: `LookupL` and `LookupML` both ended their patterns with a term after an expression variable, which makes the matcher walk the rest of the list to find the split, so a single lookup cost O(n²) where it should cost O(n); and `DsConfForCall` resolved each configuration's state *by id*, a walk of every state in the program per configuration per transition, which made the rewire pass a **cube** — inverted into a membership test over the callee's own state ids, it is a square, and `DsStateFn` is gone. Cumulative, in a debug build: `parser.ref` `GRAPH` 11.5 s → 7.8 s, `parser.ref` `RESIDUALIZE-DRIVEN` 12.2 s → 8.2 s, and the driven path on a synthetic 100-function chain **352 s → 85 s**. **New instrument**: `scripts/profile.py` wraps all 480 definitions in one-line markers and reports an exact call histogram, with `--compare` printing each function's growth ratio so a linear pass is distinguishable from a quadratic one. The honest bound: the compiler's own 122 KB source still exceeds **ten minutes** in a release build, against 0.56 s for `refal residualize-driven`. The figure holds at **~72%** — `Compile` still normalises, which is what the row deducts for — and `docs/PROGRESS.md` now carries a corrected `NEXT ACTION` with the falsified candidates removed. Tests 318 → 319 |
...
├── scripts/
│   ├── sweep.py          # Every example against `lower` and the Refal compiler
│   └── profile.py        # Exact call histogram for any compiler.ref mode
├── docs/
...
# The Refal-authored driver takes the same budget as a second argument.
