...
| 5 | Graph of states | 🔶 Partial | `refal-core` exposes deterministic sentence states, syntactic call edges, SCC detection, function-aware structural reachability cleanup, bounded ground driving, shape-aware symbolic driving, bounded Tier 1 reachability/terminal/SCC analysis, a structurally cleaned-graph Core Refal emitter, and a supported-subset symbolic residual emitter; complete Turchin driving, semantic cleaning, generalization, and driven graph residualisation remain open. The `refal differential` command now compares original checked execution with lowered/reparsed Core Refal across a representative supported corpus |
...
| Date | Change |
|---|---|
| 2026-08-18 | Post-95 incremental verification improvement: `refal differential` lowers, formats, reparses, checks, and executes Core Refal, with a corpus regression covering recursion, conditions, arithmetic, structural operations, metacode, and the Refal-authored multi-sentence compiler slice; the 95% score is unchanged because full-corpus coverage remains open |
