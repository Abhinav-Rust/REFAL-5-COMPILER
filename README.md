...
gap; heap-allocated single view field (issue [#7](../../issues/7)); §4.4's
...
| `refal-runtime` | 🔶 Broad covered builtin suite; worklist drives named calls and blocks with no fixed depth cap; projecting matcher (§2.2); Chapter 6 metacode table for `Dn`/`Up` | **Heap-allocated single view field (issue [#7](../../issues/7)); §6.4 `unknown` metacode values** |
...
dynamic dispatch (`Mu`), and the Chapter 6 metacode table (`Dn`, `Up`). Calls to any other
