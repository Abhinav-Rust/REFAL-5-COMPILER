...
**Not yet done (~12% of 1.0 target):** the *transforming* half in Refal — driving,
cleaning and generalisation still live in Rust (`refal-core`) and are not wired
into `compiler.ref`, and `driver.ref` does not exist; a program transformer that
does real work on programs rather than renaming symbols (T-1), which is the same
gap; the heap-allocated single view field, so the machine still copies term
slices instead of rewriting one flat view; §4.4's
