...
| Verified self-hosting fixpoint | 13.0% | 8.0 | C1 = C2 = C3 over the full grammar, every generation checked, and now **the driven fixpoint is a fixpoint of the Refal driver** — `the_refal_driver_reaches_a_fixpoint_on_the_compiler_itself` drives the compiler's own 132 KB source with the driver the compiler contains, checks the residue, drives it again, and requires byte-identity *and* equality with the Rust oracle's residue. That was the claim this row had been withholding credit for, and the reason given was that the driven fixpoint gated the Rust driver rather than the Refal one. It now gates the Refal one. Credit is still withheld for the default path, which normalises |
| Conformance / release evidence | 4.0% | 1.5 | A solid automated corpus; no full Classic conformance claim and no release packaging |
| **Total (1.0 target)** | **100%** | **~76%** |
...
the 100 points and carry 11.5 of the 24 deducted points. **The runtime has left the deducted
group entirely**: it was the repository's largest engineering item, and the shapes it was
still deducting for — an unbalanced rope, and a bracket that copied its contents — are both
closed and measured. Any future change to the figure has to move one of those two rows,
because they are where the product actually is.
...
the Rust oracle — over the corpus, and now over the compiler's own 132 KB source, which it
drives end to end in **36.5 s** where it previously could not finish at all, and it is a
byte-identical fixpoint of itself on that source. What it is **not** yet is a compiler whose
default path drives, because `Compile` still normalises; that is the one wiring change
standing between the driven residualizer and the compiler, and the cost that used to block it
is gone. That is the honest summary, and 76% is what it scores.
