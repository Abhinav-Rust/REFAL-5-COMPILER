...
| 5 | Graph of states | 🔶 Partial | `refal-core` exposes deterministic sentence states, syntactic call edges, SCC detection, function-aware structural reachability cleanup, bounded ground driving, shape-aware symbolic driving, bounded Tier 1 reachability/terminal/SCC analysis, a structurally cleaned-graph Core Refal emitter, and a supported-subset symbolic residual emitter; complete Turchin driving, semantic cleaning, generalization, and driven graph residualisation remain open. The `refal differential` command now compares original checked execution with lowered/reparsed Core Refal across the entire currently runnable positive runtime-conformance corpus; `refal residualize-driven` additionally exposes bounded symbolic driving, whistle states, conservative generalization counts, and checked residual source for recursive configurations |
...
| Date | Change |
|---|---|
| 2026-08-18 | Post-95 incremental Turchin improvement: `refal residualize-driven` runs bounded symbolic driving, retains visited and whistle-triggering configurations, emits checked recursive Core Refal, and records deterministic whistle/generalization metadata; the 95% score is unchanged because full configuration driving, semantic cleaning, generalized residual graph construction, and self-hosting remain open |
