...
the 100 points and carry 14.0 of the 26 deducted points. **The runtime has left the deducted
...
drives end to end in **36.5 s** where it previously could not finish at all. What it is
**not** yet is a compiler whose default path drives, because `Compile` still normalises; that
is the one wiring change standing between the driven residualizer and the compiler, and the
cost that used to block it is gone. That is the honest summary, and 74% is what it scores.
