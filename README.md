...
is the honest summary, and 66% is what it scores. The full weighting rationale is in
...
| `refal-runtime` | 🔶 Broad covered builtin suite; worklist drives named calls and blocks with no fixed depth cap; projecting matcher (§2.2); **the view field — the state is a rope of runs of shared arenas, a binding is a range of it, and a result splices its children's ropes, so execution is linear in the input's length**; Chapter 6 metacode table for `Dn`/`Up` | **A result of the shape `<F e.X> s.C` builds a left spine as deep as the nesting; §6.4 `unknown` metacode values; block sentences carrying conditions take the recursive path** |
